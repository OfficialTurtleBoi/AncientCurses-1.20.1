...
| Voodoo Doll | Initial mechanics implemented | `VoodooDollItem`; `VoodooSoulEntity` soul-link helpers; `ModClientEvents` soul tint render hook | Right-click targets a mob within 25 blocks or directly interacts with a mob to extract a Voodoo Soul. The soul is now a same-type mob clone, so it keeps the affected mob's dimensions, model, renderer, and native AI/attack behavior. Armor slots are cleared so armor does not render on the soul, and a client render hook tints soul clones teal with partial translucency. The original body is repeatedly slowed while the soul is active. Damage dealt to the soul is mirrored to the original body, and killing the soul applies Weakness II, Slowness III, and a temporary 50% armor reduction to the body. Soul clones do not drop loot or XP. |
