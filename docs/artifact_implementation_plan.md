...
| Voodoo Doll | Initial mechanics implemented | `VoodooDollItem`; `VoodooSoulEntity`; `VoodooSoulRenderer` | Right-click targets a mob within 25 blocks or directly interacts with a mob to extract a Voodoo Soul. The soul starts at the body, travels toward the player, then fights the player with a basic melee wraith-style body. The original body is repeatedly slowed while the soul is active. Damage dealt to the soul is mirrored to the original body, and killing the soul applies Weakness II, Slowness III, and a temporary 50% armor reduction to the body. Uses a 60 second vanilla cooldown. Boss-specific scaling and copied mob attack patterns still need a deeper pass. |
